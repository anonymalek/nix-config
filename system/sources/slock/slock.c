...
/* if 1 (set with -i), dont render anything on top of the screen */
int invisiblew = 0;
...
{
	int run_flag = 0;
	while ( (!run_once || !run_flag) && time_offset != 0 && timeout_command != NULL)
	{
		sleep(time_offset);
		run_flag = 1;
		system(timeout_command);
	}
	return args;
}
...
lockscreen(Display *dpy, struct xrandr *rr, int screen)
{
	...
	for (i = 0, ptgrab = kbgrab = -1; i < 6; i++) {
		...
		if (ptgrab == GrabSuccess && kbgrab == GrabSuccess) {
			if (!invisiblew) XMapRaised(dpy, lock->win);
			if (rr->active)
...
usage(void)
{
	die("usage: slock [-v] [-f] [-i] [-m message] [cmd [arg ...]]\n");
}
...
main(int argc, char **argv) {
	...
	case 'm':
		...
		break;
	case 't':
		time_offset = atoi(EARGF(usage()));
		break;
	case 'c':
		timeout_command = EARGF(usage());
		break;
	case 'i':
		invisiblew = 1;
		break;
